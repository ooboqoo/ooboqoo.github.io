...
Java会进行边界检查，运行时，如访问越界，就会抛出异常 java.lang.ArrayIndexOutOfBoundsException。

Java中二维数组就是一维数组的数组。二维数组可以是参差不齐的(即各数组长度可以不一致)，但大多数情况下，我们都会使用 MxN 的二维数组。
