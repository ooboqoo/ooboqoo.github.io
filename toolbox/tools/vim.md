...
|||
|:-----|:-------------------------------------
| `zf` | 定义折叠范围 manually define a fold up to motion
| `zd` | 删除折叠范围定义 delete fold under the cursor
| `za` | 切换 展开/折叠 toggle fold under the cursor
| `zo` | 展开 open fold under the cursor
| `zc` | 折叠 close fold under the cursor
...
Text objects (only in Visual mode or after an operator). 如，在 Normal 模式下，输入 `ci'` 就会清空 `'xxx'` 单引号范围内的内容。
...
* `s` 替换字符(删除并开始输入)
* `c` 修改(清除)
* `d` 删除(剪切)，会使用 `1`(单行或多行剪切) 或 `-`(行内部分剪切) 寄存器
* `y` 复制，会使用 `0` 寄存器
* `p` 粘贴
...
| `r`, `R`   | `r` 替换一个字符，`R` 连续替换字符直到按 `Esc`
| `s`, `S`   | 替换 substitute，`s` 删除一个字符(并开始输入)，`S` 删整行
| `c`, `C`   | 清除 change / clear，需结合定位符使用。`C` 同 `c$` 删除到行尾
| `c0` `c$` `cw` `ciw` `cii` `c1G` `cG`  | 结合定位符快速删除多行或多个字符
| `ncc`      | 删除整行 / n行
...
|||
|:-----------|:---------------------------------------------------
| `d`, `D`   | 剪切 delete，`D` 同 `d$` 删除到行尾
| `d0` `d$` `dw` `diw` `d1G` `dG` | 结合定位符快速删除(剪切)多行或多个字符
| `ndd`      | 剪切整行 / n行
| `y`, `Y`   | 复制
| `y0` `y$` `yw`  `y1G` `yG`| 结合定位符快速复制多行或多个字符
...
|||
|:-----------------|:------------------------------
| `:reg[isters]`   | 查看所有寄存器的内容
| `"<x>y` 如 `"0y` | yank into register x
| `"<x>p` 如 `"+p` | paste contents of register x
...
| `0`       | `y` last yank
| `1` ~ `9` | `dd` 最近(行级别)剪切的项目
| `-`       | 最近剪切(行内部分内容)的项目 last small (less than a line) delete，如 `dw`
| `+`       | 系统剪贴板 clipboard contents
...
| `d`      | 删除
| `~`      | 大小写切换
| `u`      | 转小写 change marked text to lowercase
| `U`      | 转大写 change marked text to uppercase
...
|:-------------|:----------------------------------------------------------------------
| `/word`      | 向下查找字符串 word
| `?word`      | 向上查找字符串 word。 &nbsp; [注] `?` 其实就是 `shift + /`
...
:%s/old/new/gc     # 替换「全文」中的 "old" 为 "new"，需要确认  `%` 表示全文查找
...
|||
|:---------|:-------------------------------------
| `i`, `I` | 进入插入模式 insert，`i` 在光标前插入，`I` 在行首非空格处开始插入
| `a`, `A` | 进入插入模式 append，`a` 在光标后插入，`A` 在行尾插入
| `o`, `O` | `o` 行后新起一行 open，`O` 行前插入一行
| `s`, `S`, `c`, `C` | 修改(删除+进入编辑模式)
| `[Esc]`  | 退出编辑模式
...
|||
|:----------|:-------------------------------------
| `:set nu / nonu` `:set number / nonumber` | 显示行号，或取消显示行号
