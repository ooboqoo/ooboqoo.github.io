...
译文：http://www.th7.cn/web/js/201601/151588.shtml

### 响应式编程

(后文中，将使用 RP 代替 Reactive Programming，私底下译者将 Reactive Programming 翻译为响应式编程。)

#### Reactive programming is programming with asynchronous data streams.

一定程度而言，RP 并不算新的概念。一连串的点击事件就是一个异步事件流，开发者可以观察这个事件流并采取一些措施。使用 Reactive 如同在此基础上开挂，极大地增强了应用范围。通常流廉价且无处不在，变量，用户输入，属性，缓存，数据结构等等都可以产生流。

**基于流的概念，RP 赋予了你一系列神奇的工具集，使用他们可以合并、创建、过滤这些流。**这就是函数式编程的魔力所在。一个流或者一系列流可以作为另一个流的输入。你可以**合并**两个流；可以**过滤**一个流并生成一个新的只包含你感兴趣的事件的流；或者将一个流中的值通过**映射**生成一个新的流。

流是一个包含时序事件(ongoing events ordered in time)的序列，事件有3种类型：(某种类型的)值、错误 以及 完成信号。

我们通过定义正常值、错误、完成信号这3种不同情况下的处理函数来**异步地**捕获发射的事件。有时我们会忽略对错误和完成信号这2种情况的处理，而仅仅关注对值的处理。对流进行监听，通常称为**订阅** subscribing ，处理流的函数是**观察者** observers，流是被观察的**主体或可观察对象** subject or observable。这就是**观察者设计模式** Observer Design Pattern。
...
a, b, c, d are emitted values  // 表示数据流发射的值
X is an error                  // 表示一个错误
| is the 'completed' signal    // 表示完成信号
---> is the timeline           // 表示时序轴
```
...
原文：https://dzone.com/refcardz/rxjs-streams
译文：https://www.oschina.net/translate/rxjs-streams
...
RxJS 是 Observable 的 Javascript 实现。有 Promise 经验，理解 Observable 会容易一些。

Promise 只能 resolve 一次，输出单值，Observable 能输出多值。
